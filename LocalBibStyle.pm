...
"$filename, $lineno: first argument to '$cmd' must contain exactly one name"
            ...
            croak
"$filename, $lineno: second argument to '$cmd' must contain exactly one string"
        ...
        sub _read_from_raw {
        	...
        	croak "$self->{lineno}: I found no rawbib content"
              unless $self->{bibtex}{rawbib}
              && $self->{bibtex}{rawbib} ne '';
          ...
          if ( $metatype == BTE_REGULAR ) {
	          ...
	          $entry->{field}{$_} =
	            $bt_entry->exists($_)
	            ? ( '"' . $bt_entry->get($_) . '"' )
	            : undef foreach @{ $self->{bibtex}{format}{field} };
        ...
        sub _read_from_files {
        	...
        	croak "$self->{lineno}: I found no bib files"
              unless $self->{bibtex}{bibfiles}
              && @{ $self->{bibtex}{bibfiles} };
        ...
        sub _command_sort {
            ...
            @{ $self->{bibtex}{cites} } =
              sort {
                $self->{bibtex}{entries}{$a}{string}{'sort.key$'}
                  cmp $self->{bibtex}{entries}{$b}{string}{'sort.key$'}
              } @{ $self->{bibtex}{cites} };
              ...
              token:
                foreach my $token ( @{$function_ar} ) {
                    ...
                    printf STDERR "$self->{lineno}: {%s} %s\n",
                      join( ' ',
                        map( $self->_format_token($_), @{ $self->{stack} } ) ),
                    ...
                    foreach
                      my $sym (qw(const entry_int entry_str integer string))
                    {
                    ...
                    if ( exists $self->{symbols}{field}{$token} ) {
                        ...
                        # if (
                        #        !defined $val
                        #     && $self->{bibtex}{bt_entry}
                        #     && ( my $xref =
                        #         $self->{bibtex}{bt_entry}->get('crossref') )
                        #   )
                        # {
                        #     my $bt_xref =
                        #       $self->{bibtex}{bt_entries}{ lc $xref };
                        #     if ($bt_xref) {
                        #         $val =
                        #           $bt_xref->exists($token)
                        #           ? ( '"' . $bt_xref->get($token) . '"' )
                        #           : undef;
                        #     }
                        #     else {
                        #         $self->_warning(
                        #             "Unknown cross reference: $xref");
                        #     }
                        # }
                        $self->_push($val);
                        next token;
                    ...
                    if ( exists $self->{symbols}{function}{$token} ) {
                        if ( $In_function{$token} ) {
                            $self->_warning(
                                "Recursive call to '$token' ignored");
                        }
                        else {
                            $In_function{$token}++;
                            _evaluate( $self,
                                $self->{symbols}{function}{$token} );
                            $In_function{$token}--;
                        }
                        next token;
                    }
    ...
    sub _format_token : method {
        ...
        $_ = $token;
        return (
              !defined $_ || $_ eq "-0" ? 'missing'
            : /^-?\d+$/ ? "#$_"
            : ref($_)   ? "{"
              . join( ' ', map $self->_format_token($_),
                grep( !/\#line/, @$_ ) )
              . "}"
            : $_
        );
    ...
    sub _function_arith {
        ...
        $self->_push(
            _check_type_warnings() ? 0 : eval "0+($arg2 $token $arg1)" );
    }
    ...
    sub _function_change_case {
        ...
        if ( $spec !~ /^[tul]$/ ) {
            $self->_warning(
                "Argument 1 of '$token' has illegal specification ($spec)");
    ...
    sub _function_eq {
        ...
        my $op = $type eq 's' ? 'eq' : '==';
        $self->_push(
            _check_type_warnings() ? 0 : eval "0+(q($arg2) $op q($arg1))" );
    }
    ...
    sub _function_format_name {
        ...
        foreach (@format) {
            if (/^PROTECT/) {
                ...
                if ( !defined $form ) {
                    $self->_warning(
                        "Invalid format specifier '$spec' for '$token'");
                    return $self->_push('""');
                }
                my $name = $name{$form};
                next unless @$name;
    ...
    sub _function_preamble {
        ...
        $self->_push( sprintf '"%s"',
            join( '', @{ $self->{bibtex}{preamble} } ) );
    }
    ...
    sub _function_text_length {
        ...
        # Count the accents
        my $length = $arg =~ s/\{ \\ (?:$Acc_char|$Acc_let[ ]) [ ]* [a-z]+ [ ]*
	(?:\}|\Z)//ogix;
        $length += $arg =~ s/\\ (?:$Acc_char|$Acc_let) [ ]* \{ [ ]* [a-z]+ [ ]*
	(?:\}|\Z)//ogix;
        $length += $arg =~ s/$Acc_sym//gox;
...
sub _function_while {
    ...
    return if _check_type_warnings();
    my $val;
    while ($self->_evaluate($cont)
        && ( $val = $self->_pop( i => $token ) )
